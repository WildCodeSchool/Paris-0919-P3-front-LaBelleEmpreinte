...
export default function CreateInitiatives(props) {
  ...
  const handleModification = () => {
    axios.put(`http://localhost:4000/admin/initiatives_maj/${id}`, articleDataForBack)
      .then(res => setRes(res))
    axios.put(`http://localhost:4000/admin/initiatives/${id}`, articleDataForBack)

    // .then(error())
  }

  const handleSupress = () => {
    axios.delete(`http://localhost:4000/admin/initiatives_maj/${id}`)
  }
  ...
  return (
    <>
      ...
      < Modal isOpen={modifVisible} toggle={handleClickModif} className="" >
        <ModalHeader toggle={handleClickModif}>L'initiative a bien été modifiée</ModalHeader>
        <ModalBody>
          <div className="menu-modal">
            <Link to="/admin/afficher/initiatives"><input type="button" value="OK" /></Link>
          </div>
        </ModalBody>
      </Modal >
      ...
      < Modal isOpen={visible} toggle={handleClick} className="" >
        <ModalHeader toggle={handleClick}>Souhaitez vraiment supprimer l'initiative?</ModalHeader>
        <ModalBody>
          <div className="menu-modal">
            <Link to="/admin/afficher/initiatives"><input type="button" value="Confirmer" onClick={() => handleSupress()} /></Link>
          </div>
        </ModalBody>
      </Modal >
